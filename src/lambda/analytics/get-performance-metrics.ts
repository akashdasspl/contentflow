...
/**
 * Lambda function to get performance metrics and KPIs
 * GET /analytics/performance
 */
export const handler = async (event: APIGatewayEvent): Promise<APIGatewayResponse> => {
  try {
    console.log('Get performance metrics request:', JSON.stringify(event, null, 2));
    ...
    const performanceMetrics = await analyticsDashboardService.getPerformanceMetrics(
      authResult.userId,
      dateRange
    );
